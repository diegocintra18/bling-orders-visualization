...
export async function POST(request: NextRequest) {
  try {
    ...
    const { accountId, clientId, clientSecret, code, redirectUri } = await request.json();

    if (!accountId || !clientId || !clientSecret || !code || !redirectUri) {
    ...
    return NextResponse.json(
      { error: 'Erro ao obter autorização do Bling' },
      { status: 500 }
    );
  }
}
